...
if __name__ == '__main__':
    with open('TripleLSTM-{}-{}.csv'.format(datetime.datetime.now().date(), datetime.datetime.now().second), 'w',
              newline='') as csv_file:
        header = ['Data', 'Accuracy', 'Precision', 'Recall', 'F-Measure']
        writer = csv.writer(csv_file)
        writer.writerow(header)
        for each in range(1, 16):
            file_path = 'Dataset/3class/data{}.csv'.format(each)
            ...
            print("\n\n({}/15) Triple Classification LSTM:".format(each))
